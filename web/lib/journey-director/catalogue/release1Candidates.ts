 ...
 * unchanged.
 *
 * `R1.2-WS3-IMP-02-EBC-RAD` (WP-5) replaced this module's previously
 * hand-authored `EMOTION_BY_LABEL`/`THEMES_BY_LABEL`/`TRAVELLER_BY_LABEL`
 * constant tables with the generated, operational-layer-validated tables
 * produced by `web/scripts/journey-intelligence/generateLabelMappings.ts`
 * (folded into `metadata.json`, exposed here via `runtimeJourneyIntelligence`
 * — the same fail-closed-verified source every other constant in this file
 * already reads from). The mapping *values* are unchanged from before this
 * EBC; see `labelMappingSource.ts` for why they remain a governed, curated
 * input rather than something derived purely from workbook text, and see
 * `docs/09-Development/R1.2-WS3-IMP-02-EBC-RAD-WP5-Implementation.md` for
 * the comparison/reachability reporting this generation now produces.
 */
...
// Generated by `generateLabelMappings.ts` (WP-5) into `metadata.json`,
// validated against the operational layer's current Emotional Goals /
// Desired Experiences / Traveller Types sheets on every regeneration — see
// this file's header comment. The cast narrows JSON-sourced string data to
// this file's literal-union types, matching the same pattern
// `loadWorkbook.ts` already uses for workbook-sourced string fields (e.g.
// `get("Travel Scope") as TravelScope`).
const EMOTION_BY_LABEL = runtimeJourneyIntelligence.metadata.labelMappings
  .emotionByLabel as Readonly<Record<string, EmotionId>>;
const THEMES_BY_LABEL = runtimeJourneyIntelligence.metadata.labelMappings
  .themesByLabel as Readonly<Record<string, readonly ThemeId[]>>;
const TRAVELLER_BY_LABEL = runtimeJourneyIntelligence.metadata.labelMappings
  .travellerByLabel as Readonly<Record<string, TravellerType>>;
