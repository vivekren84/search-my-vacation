 ...
 *
 * `R1.2-WS3-IMP-02-EBC-RAD` (WP-5) added Controlled Vocabulary Mapping
 * Generation: the `EMOTION_BY_LABEL`/`THEMES_BY_LABEL`/`TRAVELLER_BY_LABEL`
 * tables `release1Candidates.ts` used to hand-author are now generated by
 * `generateLabelMappings.ts` (folded into `metadata.json`) and validated
 * against the operational layer, with comparison and reachability findings
 * reported. It never fails the run and never extends either vocabulary.
 */
...
export async function runGeneration(
  ...
  logEvent("Generating Label Mappings", "COMPLETE", {
    comparisonFindings: artifacts.metadata.labelMappings.comparison.length,
    reachability: artifacts.metadata.labelMappings.reachability.map(
      (entry) => `${entry.vocabulary}:${entry.reachablePercentage}%`,
    ),
  });
  ...
  if (options.reportPath) {
    await writeGenerationReport({
      ...
      validation,
      kbReconciliation,
      labelMappings: artifacts.metadata.labelMappings,
