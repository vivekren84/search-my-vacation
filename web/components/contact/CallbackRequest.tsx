  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        if (form.consent && !dateError) window.location.assign(href);
      }}
    >
      <h2 className="font-serif text-3xl tracking-[-.035em]">Request a Callback</h2>
      <p className="mt-4 text-sm leading-7 text-[#705c47]">
      ...
      <div className="mt-7 grid gap-5 sm:grid-cols-2">
        ...
        <label className="text-sm font-semibold">Phone number<input required type="tel" value={form.phone} onChange={(event) => update("phone", event.target.value)} className="mt-2 min-w-0 w-full rounded-xl border border-[#d8c4a7] px-3 py-3 font-normal" /></label>
      ...
      <label className="mt-5 block text-sm font-semibold">Travel message <span className="font-normal text-[#705c47]">(optional)</span><textarea value={form.message} onChange={(event) => update("message", event.target.value)} className="mt-2 min-h-28 min-w-0 w-full rounded-xl border border-[#d8c4a7] px-3 py-3 font-normal" /></label>
      <label className="mt-6 flex gap-3 text-sm leading-6 text-[#705c47]"><input required type="checkbox" checked={form.consent} onChange={(event) => update("consent", event.target.checked)} className="mt-1 size-4" />I agree that Search My Vacation may use these details to respond to my request, as explained in the <a className="underline" href="/privacy-policy">Privacy Policy</a>.</label>
      <button className="mt-7 rounded-full bg-[#a86f29] px-6 py-3 text-sm font-bold text-white disabled:bg-[#cdbca6]" disabled={!form.consent || Boolean(dateError)}>Continue on WhatsApp <span aria-hidden="true">→</span></button>
      <p className="mt-4 text-xs leading-5 text-[#705c47]">This opens WhatsApp with your details prefilled. No callback request is sent until you choose to send that message.</p>
    </form>
  );
}
