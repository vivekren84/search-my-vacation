...
export default function JourneyPassport() {
    ...
    return <main className="journey-passport-closure-page">
      <section className={`journey-passport-closure is-${closureStage}`} aria-label="Journey Passport issuance">
        ...
        <div className="passport-closure-mark" aria-hidden="true">
          ...
          <Image className="passport-closure-stamp-image" src="/brand/master/smv-passport-stamp-master.png" alt="" width={640} height={640} priority unoptimized />
        </div>
        {issued ? <div className="journey-passport-issued-content">
          ...
          <form onSubmit={continueToDirector} className="journey-passport-reveal mx-auto mt-7 max-w-md text-left" noValidate>
            <h2 className="text-center text-xl font-semibold text-[#392719]">Keep your journey connected</h2>
            <p className="mt-2 text-center text-sm leading-6 text-[#705c47]">Confirm your details so your Journey Director can keep this Passport connected to you.</p>
            <label htmlFor="issued-passport-name" className="mt-5 block text-sm font-semibold text-[#4c3825]">Name<input id="issued-passport-name" type="text" autoComplete="name" maxLength={80} value={passport.state.name} onChange={(event) => update({ name: event.target.value.replace(/[\r\n]+/g, " ") })} className="mt-2 min-h-12 w-full rounded-xl border border-[#d8c4a7] bg-white px-4 text-base focus:outline-2 focus:outline-offset-2 focus:outline-[#a86f29]" /></label>
            <label htmlFor="issued-passport-mobile" className="mt-4 block text-sm font-semibold text-[#4c3825]">Mobile number<input id="issued-passport-mobile" type="tel" autoComplete="tel" inputMode="tel" maxLength={24} value={passport.state.mobile} onChange={(event) => update({ mobile: event.target.value.replace(/[^\d+()\-\s]/g, "") })} aria-describedby="issued-passport-privacy issued-passport-error" className="mt-2 min-h-12 w-full rounded-xl border border-[#d8c4a7] bg-white px-4 text-base focus:outline-2 focus:outline-offset-2 focus:outline-[#a86f29]" placeholder="e.g. +91 98765 43210" /></label>
            ...
            <p id="issued-passport-privacy" className="mt-4 text-xs leading-5 text-[#796653]">Used only to connect this Passport with your planning conversation. We do not sell your contact details.</p>
            ...
            <button type="button" onClick={reviewPassport} disabled={contactSubmission !== "idle"} className="mx-auto block !mt-3 !bg-transparent !text-[#704b27] !shadow-none">Review my Passport</button>
          </form>
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#fbf7ef] text-[#2d2117]">
    <header className="relative z-20 grid min-h-[5.5rem] grid-cols-[auto_1fr] items-center border-b border-white/20 bg-[#20150f] px-[var(--layout-gutter)] text-white md:min-h-[6.5rem] md:grid-cols-[1fr_auto_1fr]">
    ...
    <section className={`layout-container layout-container--wide relative ${passport.moment.id === "welcome" ? "flex min-h-[calc(100svh-5.5rem)] items-center py-[clamp(1.75rem,5vh,4rem)] md:min-h-[calc(100svh-6.5rem)]" : "pb-[max(4rem,env(safe-area-inset-bottom))] lg:pb-20"}`}>
      ...
      <div key={passport.moment.id} className={`journey-passport-enter mx-auto w-full ${passport.moment.id === "welcome" ? "max-w-6xl" : "max-w-7xl"}`}>
        ...
        {passport.moment.id === "pace-and-timing" ? <PaceAndTimingMoment headingRef={headingRef} moment={passport.moment} state={passport.state} onUpdate={update} onToggle={(value) => passport.dispatch({ type: "toggle-style", value })} /> : null}
