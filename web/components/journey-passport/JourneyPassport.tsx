...
import { resolveJourneyEntryPreselection } from "@/lib/journey-passport/entry-context";
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#FFFDFC] text-[#2A211C]">
    ...
    <section className={`layout-container layout-container--wide relative ${passport.moment.id === "welcome" ? "flex min-h-[calc(100svh-5.5rem)] items-center py-[clamp(1.75rem,5vh,4rem)] md:min-h-[calc(100svh-6.5rem)]" : "pb-[max(4rem,env(safe-area-inset-bottom))] lg:pb-20"}`}>
      ...
      <div key={passport.moment.id} className={`journey-passport-enter mx-auto w-full ${passport.moment.id === "welcome" ? "max-w-6xl" : "max-w-7xl"}`}>
        ...
        {passport.moment.id === "companions" ? <SingleChoiceMoment headingRef={headingRef} moment={passport.moment} value={passport.state.companion} showEntryAdvisory={entryPreselection?.moment === "companions"} onChange={(companion) => update({ companion: passport.state.companion === companion ? "" : companion })} /> : null}
        {passport.moment.id === "dream-journey" ? <SingleChoiceMoment headingRef={headingRef} moment={passport.moment} value={passport.state.dreamJourney} showEntryAdvisory={entryPreselection?.moment === "dream-journey"} onChange={(dreamJourney) => update({ dreamJourney: passport.state.dreamJourney === dreamJourney ? "" : dreamJourney })} /> : null}
        {passport.moment.id === "pace-and-timing" ? <PaceAndTimingMoment headingRef={headingRef} moment={passport.moment} state={passport.state} showEntryAdvisory={entryPreselection?.moment === "pace-and-timing"} onUpdate={update} onToggle={(value) => passport.dispatch({ type: "toggle-style", value })} /> : null}
