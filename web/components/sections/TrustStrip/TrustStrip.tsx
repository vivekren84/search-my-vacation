...
import { GlobeIcon, GoogleGIcon, RouteIcon, UsersIcon } from "@/components/icons/TrustIcons";
    ...
    >
      <div className="layout-container">
        <div className="mx-auto flex max-w-[62rem] flex-col divide-y divide-[#e4d2b5] rounded-[1.5rem] border border-[#e4d2b5] bg-white shadow-[0_8px_24px_rgba(91,55,18,.05)] sm:flex-row sm:divide-x sm:divide-y-0">
          ...
          >
            <span className="grid size-9 place-items-center rounded-full border border-[#e4d2b5] bg-white">
          ...
          {staticItems.map(({ key, Icon, value, label, subLabel }) => (
            <div
              key={key}
              className="flex flex-1 flex-col items-center gap-2 px-6 py-6 text-center sm:py-7"
            >
              <span className="grid size-9 place-items-center rounded-full bg-[#f5e4c5] text-[#986328]">
