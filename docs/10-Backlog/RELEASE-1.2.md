...
| Version | 1.21 |
| Status | Draft — Active Implementation (Not Yet Ready for Formal Closure — see "Release 1.2 Closure Readiness" below Section 3) |
...
| 1.17 | 31-Aug-2026 | Tiger | R1.2-WS7-07 | Release 1.2 Final Tracker Reconciliation (`EBC-R1.2-WS7-07`, Scope: `docs/10-Backlog/RELEASE-1.2.md` only). Per this card's Workspace Readiness Check, cross-referenced the last independent closure assessment (`EBC-R1.2-WS8-01`, "Not Ready") against current repository evidence rather than assuming it stale. Found: (1) genuinely resolved since `WS8-01` — Workstream 6 formally **Accepted by the Product Owner** (`EBC-R1.2-WS6-14-VIVEK`), its Operational Release Gate migration/GeoNames-import both independently confirmed applied (repository evidence: `supabase/migrations/20260823150000_geo_places_geo_aliases.sql` plus three follow-on WS3 performance migrations present; `web/data/geonames/` populated); Workstream 5's Keerthi functional QA (`WS5-QA-01`, Passed with Observations) and Sri traveller validation (`WS5-SRI-01`) both complete, and the OTP-expiry/SMS-wording mismatch resolved and **committed** (`3280b93`, confirmed by `WS5-QA-03` smoke validation); Release 1.3 backlog carry-forward reconciliation complete (`EBC-R1.2-WS7-06`, committed `2e1ff3b`). (2) **still open, independently reverified, not marked complete without evidence**: Workstream 8 (release-wide QA & regression) has not started — 0 commits, 0 new documents since `WS8-01`; Workstream 4's status remains disputed (tracker records 0%; an out-of-repository evidence chain describing it as substantially complete has still not been committed or independently re-verified against commit `0bbac73`); the six fabricated `WS7-IMP-01`–`04`/`WS7-AUD-01`/`WS7-GOV-01` citations `WS8-01` identified remain verbatim, unresolved, in `GOVERNANCE-MAP.md`, `DECISION-LOG.md`, `30-Governance/README.md`, `03-ADR/DECISIONS.md`, `PROJECT-HISTORY.md` and `RELEASE-1.3-BACKLOG.md` — correcting them requires edits outside this card's stated scope and is recommended as its own dedicated Tiger card; `[SMV-DBG]` diagnostics remain present in three WS5 files (confirmed by direct repository search), contrary to earlier "pending removal" text. Consequently **Workstream 7 is not marked Complete by this update** — its own fabricated-citation defect and three genuinely outstanding tasks (07.02, 07.04, 07.06) remain open. Updated: dashboard task-count breakdown (49 Complete / 1 In Progress / 17 Ready for QA / 61 Proposed, reconciled against Section 6 — the 13-task increase in Complete over `WS8-01`'s own 36/1/17/74 recount is entirely Workstream 6's task table, flipped to Complete on the strength of its Product Owner Acceptance); approved-decision count (added `DEC-R1.2-027` WS6 acceptance and `DEC-R1.2-028` OTP-expiry resolution); Open Decision `OD-4` added to Section 8 as its own row; three new Section 10 risks recorded for the still-open WS4/WS7/WS8 items; unified the three disagreeing version/date stamps (Document Information, Section 1's own Document Version/Last Updated fields, Section 12's checklist) to this version. Added a new "Release 1.2 Closure Readiness" note directly below Section 3. No task-table row was flipped to Complete without a whole-workstream Product Owner Acceptance record (Workstream 6) or, failing that, left exactly as recorded, per this card's own "do not mark unresolved items complete without evidence" instruction — Workstream 4 and Workstream 5's remaining `Ready for QA`/`Proposed` rows are therefore deliberately unchanged, even though WS5's own narrative above records genuine QA/traveller-validation progress, pending a line-by-line reconciliation this card's scope did not extend to. No code, configuration, or Release 1.3 prioritisation was performed, per this card's explicit Out of Scope. |
...
| Number of Completed Tasks | 60 of 128 | All 19 Workstream 1 tasks, all 13 Workstream 2 tasks, all 13 Workstream 6 tasks (R1.2-06.01–06.13, flipped to Complete on the strength of the Product Owner's Workstream 6 Acceptance — see Section 5), Workstream 5's `R1.2-05.18`–`05.20` (3), `R1.2-07.05`, and all eleven Workstream 4 tasks — `R1.2-04.10` (`EBC-R1.2-WS8-02`), `R1.2-04.01`/`04.03`/`04.04` (`EBC-R1.2-WS4-05`), and `R1.2-04.02`/`04.05`–`04.09`/`04.11` (new, `EBC-R1.2-WS4-06` — see Section 17.4) — are Complete — 19+13+13+3+1+11 = 60. |
| Task Status Breakdown | 60 Complete · 1 In Progress · 17 Ready for QA · 50 Proposed (of 128) | Reconciled directly against the six per-workstream Section 6 task tables by `EBC-R1.2-WS8-01` (36/1/17/74), re-reconciled by `EBC-R1.2-WS7-07` after flipping Workstream 6's 13 tasks to Complete (49 Complete; 61 Proposed), re-reconciled by `EBC-R1.2-WS8-02` after flipping `R1.2-04.10` to Complete (50 Complete; 60 Proposed), re-reconciled by `EBC-R1.2-WS4-05` after flipping `R1.2-04.01`/`04.03`/`04.04` to Complete on independently-verified repository evidence (50+3=53 Complete; 60-3=57 Proposed), and re-reconciled again by this update (`EBC-R1.2-WS4-06`) after flipping `R1.2-04.02`/`04.05`–`04.09`/`04.11` to Complete on independently-verified repository evidence (53+7=60 Complete; 57-7=50 Proposed). The previous dashboard figure ("33 Complete · 0 Approved · 95 Proposed") had omitted the "Ready for QA" and "In Progress" categories entirely — corrected at `EBC-R1.2-WS7-07`. |
...
| Priority | P1 |
| Status | 🟢 All Eleven Tasks Complete — Ready for Product Owner Acceptance |
...
**Update, 01-Sep-2026 (`EBC-R1.2-WS4-06`):** a further repository evidence synchronisation found a previously-unexamined evidence chain -- `EBC-R1.2-WS4-AUD-01` (Rad, engineering audit), `EBC-R1.2-WS4-IMP-01` (Rad, implementation), `EBC-R1.2-WS4-QA-01` (Keerthi, functional QA) and `EBC-R1.2-WS4-SRI-01` (Sri, traveller validation), all dated 30-Aug-2026 and titled against this workstream's own name -- that directly resolves `04.02` and all five remaining Keerthi acceptance/regression tasks (`04.05`-`04.09`, `04.11`). All eleven WS4 tasks are now Complete in Section 6.4. This still does not formally close Workstream 4: no Product Owner acceptance record exists for it, consistent with how Workstream 6 was not counted as a completed workstream until `EBC-R1.2-WS6-14-VIVEK`. **Workstream 4's Status field is updated to "All Eleven Tasks Complete -- Ready for Product Owner Acceptance."** The separate, larger, previously-unreconciled "Experience Refinement" delivery chain flagged in the note above is confirmed to be a different scope, not the source of this evidence, and remains its own outstanding reconciliation -- see Section 17.2 and 17.6. See Section 17 for the complete evidence mapping matrix.
...
| Priority | P1 |
| Status | 🟢 Functional QA & Traveller Validation Complete – Pending Product Acceptance (Keerthi's functional QA **Passed with Observations**, `EBC-R1.2-WS5-QA-01`, 30-Aug-2026; Sri's traveller experience validation **complete**, `EBC-R1.2-WS5-SRI-01`, 30-Aug-2026; OTP-expiry/SMS-wording mismatch **resolved and committed**, `3280b93`, confirmed by a smoke validation, `EBC-R1.2-WS5-QA-03`; these three are recorded in the Claude Project, not yet committed to `docs/09-Development/` — see the "Functional QA & Traveller Validation Complete" update below) |
...
- [x] This document (`RELEASE-1.2.md`) is current at Version 1.21
...
- [ ] Open decisions (Section 8) are resolved or explicitly carried to a future release (now includes OPEN-R1.2-007–009 from R1.2-014 and OD-4)
